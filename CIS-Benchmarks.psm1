Function Invoke-CIS-Benchmarks {

    <#
    .SYNOPSIS

        Invoke-CIS-Benchmarks - Checks and hardens your Windows configuration
   
        Author:  Antonio Camelo
        License: MIT
    ...
    .DESCRIPTION
        ...
        CIS-Benchmarks supports hardening of a Windows system. The configuration of the system is
        retrieved and assessed using a finding list. In addition, the system can be hardened according
        to predefined values. CIS-Benchmarks reads settings from the registry and uses other modules
        to read configurations outside the registry.


    .PARAMETER FileFindingList

        Path to a finding list in CSV format. CIS-Benchmarks has one list each for machine and user settings.


    .PARAMETER Mode

        The mode Config only retrieves the settings, while the mode Audit performs an assessment of the settings.
        The mode Benchmark hardens the system according to recommendations of the CIS-Benchmarks list.
    ...
    .PARAMETER Log
        ...
        name is assigned by CIS-Benchmarks itself and the file is stored in the same directory as the script.
    ...
    .PARAMETER Report
        ...
        The file name is assigned by CIS-Benchmarks itself and the file is stored in the same directory as the script.
    ...
    .PARAMETER SkipRestorePoint
        ...
        Do not create a System Restore Point in Benchmark mode. CIS-Benchmarks strongly recommends to backup your system before running Hail Mary. However,
        creating can be skipped, for example, if Benchmark is executed several times in a row. By default, Windows allows a restore point every 24 hours.
        Another reason is when CIS-Benchmarks is run as a user and thus lacks privileges.
    ...
    .EXAMPLE
        Invoke-CIS-Benchmarks -Mode Audit -Log -Report

        CIS-Benchmarks performs an audit, saves the results and creates a log file

    .EXAMPLE
        Invoke-CIS-Benchmarks -FileFindingList finding_list_0x6d69636b_user.csv -SkipMachineInformation

        CIS-Benchmarks performs an audit with a specific list and does not show machine information

    .EXAMPLE
        Invoke-CIS-Benchmarks -Mode Config -Report -ReportFile C:\tmp\my_CIS-Benchmarks_report.csv

        CIS-Benchmarks uses the default list, and saves the results in a specific file

    .EXAMPLE
        Invoke-CIS-Benchmarks -Filter { $_.Severity -eq "Medium" }

        CIS-Benchmarks uses the default list, and checks only tests with the severity Medium
    #>
    ...
    Param (
        ...
        [ValidateSet("Audit", "Config", "Benchmark")]
        [String]
        $Mode = "Audit",
    ...
    If ($Backup -and $BackupFile.Length -eq 0) {
        $BackupFile = "backup_" + $Hostname + "_" + $ListName + "-$FileDate.csv"
    }
    $ReportAllResults = @()
    $BackupAllResults = @()
    ...
    Write-Output "`n"
    Write-Output ""
    Write-Output "CIS-Benchmarks $CIS-BenchmarksVersion"
    Write-Output "`n"
    Write-ProtocolEntry -Text "Starting CIS-Benchmarks" -LogLevel "Info"
    ...
    If ($WinSystemLocale.Name -ne "en-US" -and -not($SkipLanguageWarning)) {
        ...
        $Message = "CIS-Benchmarks was developed for the system language 'en-US'. This system uses '" + $WinSystemLocale.Name + "' Language-dependent analyses can sometimes produce false results. Please create an issue if this occurs."
    ...
    If ($Mode -eq "Audit" -or $Mode -eq "Config") {
        ...
        # A CSV finding list is imported. CIS-Benchmarks has one machine and one user list.
        If ($FileFindingList.Length -eq 0) {
        ...
        If ($Filter) {
            ...
            If ($FindingList.Length -eq 0) {
                $Message = "Your filter did not return any results, please adjust the filter so that CIS-Benchmarks has something to work with."
                Write-ProtocolEntry -Text $Message -LogLevel "Error"
                Break
            }
        }
        ...
        ForEach ($Finding in $FindingList) {
            ...
            # to a file, which means CIS-Benchmarks must create a temporary file
            # and afterwards delete it. CIS-Benchmarks is very orderly.
            #
            ...
            ElseIf ($Finding.Method -eq 'auditpol') {
                ...
                try {
                    ...
                    $TempFileName = [System.IO.Path]::GetTempPath() + "CIS-Benchmarks_auditpol-" + $(Get-Date -Format yyyyMMdd-HHmmss) + ".csv"
    ...
    # CIS-Benchmarks configures all settings in a finding list file.
    # Even though CIS-Benchmarks works very carefully, please only
    ...
    Elseif ($Mode -eq "Benchmark") {
        ...
        If (-not($SkipRestorePoint)) {
            ...
            Try {
                Checkpoint-Computer -Description 'CIS-Benchmarks' -RestorePointType 'MODIFY_SETTINGS' -ErrorAction Stop -WarningAction Stop
            } catch {

                $Message = "Creating a system restore point failed. Use -SkipRestorePoint to run Benchmark anyway. Be careful!"
    ...
    Write-Output "`n"
    Write-ProtocolEntry -Text "Result CIS Benchmarks is done" -LogLevel "Info"
    ...
    If ($Mode -eq "Audit") {
        ...
        If ($ScoreTotal -ne 0 ) {
            $CIS-BenchmarksScore = ([int] $ScoreAchived / [int] $ScoreTotal) * 5 + 1
        }
        $CIS-BenchmarksScoreRounded = [math]::round($CIS-BenchmarksScore, 2)

        # Overwrite CIS-Benchmarks Score if no finding is passed
        If ($StatsPassed -eq 0 ) {
            $CIS-BenchmarksScoreRounded = 1.00
        }

        If ($Script:StatsError -gt 0) {
            Write-ProtocolEntry -Text "During the execution of CIS-Benchmarks errors occurred due to missing admin rights or tools. For a complete result, these errors should be resolved. Total errors: $Script:StatsError" -LogLevel "Error"
        }

        Write-ProtocolEntry -Text "Your CIS Benchmarks score is: $CIS-BenchmarksScoreRounded. CIS Benchmarks Statistics: Total checks: $StatsTotal - Passed: $StatsPassed, Low: $StatsLow, Medium: $StatsMedium, High: $StatsHigh." -LogLevel "Info"
    }
    Write-Output "`n"
}

Export-ModuleMember -Function Invoke-CIS-Benchmarks
